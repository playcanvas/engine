...
class BottomBar extends TypedComponent {
    /** @type {State} */
    state = {
        ...
        // @ts-ignore
        collapsed: localStorage.getItem('bottomBarCollapsed') === 'true' || window.top.innerWidth < MIN_DESKTOP_WIDTH,
    ...
    componentDidMount() {
        ...
        const bottomBarHeader = bottomBar.querySelector('.pcui-panel-header');
        if (!bottomBarHeader) {
            return;
        }
        bottomBarHeader.onclick = () => this.toggleCollapse();
        ...
        this.state.observer.on('largeThumbnails:set', () => {
            ...
            for (let i = 0; i < navItems.length; i++) {
                ...
                if (navItemDistance < minTopNavItemDistance) {
                    ...
                    bottomBar.classList.toggle('small-thumbnails');
                    nav.scrollIntoView();
                    break;
                }
            }
