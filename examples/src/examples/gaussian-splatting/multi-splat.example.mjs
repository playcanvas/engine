...
assetListLoader.load(() => {
    ...
    guitar.gsplat.material.getShaderChunks(shaderLanguage).set('gsplatModifySplatVS', files[customShaderFile]);
    ...
    data.on('shader:set', () => {
        ...
        if (useCustomShader) {
            mat1.getShaderChunks(shaderLanguage).set('gsplatModifySplatVS', vs);
        } else {
            mat1.getShaderChunks(shaderLanguage).delete('gsplatModifySplatVS');
        }
        mat1.update();

        const mat2 = skull.gsplat.material;
        if (useCustomShader) {
            mat2.getShaderChunks(shaderLanguage).set('gsplatModifySplatVS', vs);
        } else {
            mat2.getShaderChunks(shaderLanguage).delete('gsplatModifySplatVS');
        }
