...
const generateSkyboxCubemap = (source) => {
    const reprojectToCubemap = (src, size) => {
        ...
        reprojectTexture(src, faces);

        return faces;
    };

    if (source.cubemap) {
        if (source.type === TEXTURETYPE_DEFAULT || source.type === TEXTURETYPE_RGBM) {
            // source is cubemap and format is acceptible, use it directly
            return source;
        } else {
            // source cubemap must be rgbm or default to be used on the skybox
            return reprojectToCubemap(source, source.width);
        }
    } else {
        // reproject equirect to cubemap for skybox
        return reprojectToCubemap(source, source.width / 4);
    }
};
...
const generatePrefilteredCubemaps = (source, mipmaps = true) => {
...
const generatePrefilteredCubemap = (source) => {
