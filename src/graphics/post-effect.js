 ...
 * @property {VertexBuffer} vertexBuffer The vertex buffer for the fullscreen quad. Used when
 * calling {@link drawFullscreenQuad}. [read only].
 * @property {Shader|null} shader The shader definition for the fullscreen quad. Needs to be set by
 * the custom post effect (default is null). Used when calling {@link drawFullscreenQuad}.
 * @property {boolean} needsDepthBuffer The property that should to be set to `true` (by the custom
 * post effect) if a depth map is necessary (default is false).
 */
class PostEffect {
    /**
