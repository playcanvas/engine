Object.assign(pc, function () {
    ...
    ShaderGraphNode.prototype.setParam = function (type, name, value) {
        this.param.type=type;
        this.param.name=name;
        this.param.value=value;

        this.outputName.push(name);
    ...
    ShaderGraphNode.prototype.getOutputs = function (graphParamList, graphFuncList, graphTmpVarList, graphNodeStringList) {
        var isRoot=false;
        ...
        if (this.param)
        {
            ...
            if (this.outputTempVarIndex[0]==-1) //if already set, skip
            {
                ...
                nodeString += this.outputType[0]+' '+this.outputName[0]+'_'+this.outputTempVarIndex[0]+';\n';
            }       
        ...
        if (this.functionString)
        {
            ...
            var inputGlobalIndex=[];
            for (var i=0;i<this.inputNode.length;i++)
            {
                if (this.inputNode[i].outputTempVarIndex[inputOutputIndex[i]]==-1)
                {
                    this.inputNode[i].getOutputs(graphParamList, graphFuncList, graphTmpVarList, graphNodeStringList);
                }
                inputGlobalIndex[i]=this.inputNode[i].outputTempVarIndex[inputOutputIndex[i]];
            }
            ...
            for (var i=0;i<this.inputNode.length;i++)
            {
                nodeString+=graphTmpVarList[inputGlobalIndex[i]][1]+'_'+inputGlobalIndex[i];
                if (this.outputTempVarIndex.length>0 || i<(this.inputNode.length-1)) nodeString+=', ';
            }
            for (var i=0;i<this.outputTempVarIndex.length;i++)
            {
