export default /* glsl */`
// Anisotropic GGX
float calcLightSpecular(float tGlossiness, vec3 tNormalW, vec3 viewDir, vec3 h, vec3 lightDirNorm, mat3 tbn) {
    ...
    float NoL = dot(tNormalW, -lightDirNorm);
...
float getLightSpecular(vec3 h, vec3 reflDir, vec3 worldNormal, vec3 viewDir, float gloss, mat3 tbn) {
    return calcLightSpecular(glossiness, worldNormal, viewDir, h, tbn);
}
`;
