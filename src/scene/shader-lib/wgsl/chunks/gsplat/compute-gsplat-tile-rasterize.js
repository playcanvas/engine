export const computeGsplatTileRasterizeSource = /* wgsl */`
...
fn main(
    ...
    let tileIdx = wid.y * uniforms.numTilesX + wid.x;
    let tStart = tileRanges[tileIdx * 2u];
    let tEnd = tileRanges[tileIdx * 2u + 1u];

    let basePixel = vec2u(wid.x * 16u + lid.x * 2u, wid.y * 16u + lid.y * 2u);
    ...
    for (var batch: u32 = 0u; batch < numBatches; batch++) {
        ...
        // Reset done counter for this batch
        if (localIdx == 0u) {
        ...
        if (batchOffset < tileCount) {
            let cacheIdx = sortedValues[tStart + batchOffset];
        ...
        // Count done threads and check if entire workgroup is saturated
        if (threadDone) {
    ...
    // Write the 2x2 pixel quad to the output texture (premultiplied alpha).
    // Each thread covers 4 pixels; bounds-check handles non-multiple-of-2 screen edges.
