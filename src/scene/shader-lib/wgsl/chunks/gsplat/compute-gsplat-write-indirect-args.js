...
// arguments (projector, sort), numSplats for the vertex shader,
...
struct WriteArgsUniforms {
    ...
    dispatchSlotBase: u32,      // slot index of the projector dispatch; sort slots follow
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
    ...
    // Layout: slot[base + 0] = projector, slot[base + 1..base + 1 + sortSlotCount] = sort.
    let projectorSlot = uniforms.dispatchSlotBase;
    let projectorOffset = projectorSlot * 3u;

    let projectorWorkgroupCount = (count + {PROJECTOR_WORKGROUP_SIZE}u - 1u) / {PROJECTOR_WORKGROUP_SIZE}u;
    let projectorDim = calcDispatch2D(projectorWorkgroupCount, {MAX_WORKGROUPS_PER_DIM}u);
    indirectDispatchArgs[projectorOffset + 0u] = projectorDim.x;
    indirectDispatchArgs[projectorOffset + 1u] = projectorDim.y;
    indirectDispatchArgs[projectorOffset + 2u] = 1u;
    ...
    writeSortIndirectArgs(projectorSlot + 1u, count, uniforms.sortIndirectInfo);
