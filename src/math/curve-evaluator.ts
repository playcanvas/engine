import { CURVE_CARDINAL, CURVE_CATMULL, CURVE_LINEAR, CURVE_SMOOTHSTEP, CURVE_SPLINE, CURVE_STEP } from './constants';
...
class CurveEvaluator {
    ...
    _calcTangents(keys: Array<number[]>, index: number): void {
        var a;
        var b = keys[index];
        var c = keys[index + 1];
        var d;
    ...
    _evaluateHermite(p0: number, p1: number, m0: number, m1: number, t: number): number {
        var t2 = t * t;
        var twot = t + t;
        var omt = 1 - t;
        var omt2 = omt * omt;
