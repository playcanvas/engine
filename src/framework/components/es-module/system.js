...
class ESModuleComponentSystem extends ComponentSystem {
    ...
    initializeComponentData(component, data) {
        ...
        if (data.hasOwnProperty('order') && data.hasOwnProperty('modules')) {
            component._modulesData = data.modules;

            for (let i = 0; i < data.order.length; i++) {
                component.create(data.order[i], {
                    enabled: data.modules[data.order[i]].enabled,
                    attributes: data.modules[data.order[i]].attributes,
