...
class RigidBodyComponentSystem extends ComponentSystem {
    ...
    _afterStepSimulation(dt) {
        ...
        for (i = 0, len = dynamic.length; i < len; i++) {
            dynamic[i]._updateDynamic(!this.app.useFixedTimeForPhysics);
        }
    ...
    onPhysicsUpdate(dt) {
        ...
        if (!this.app.useFixedTimeForPhysics || this._lastFixedTimeStep === 0) {
            return;
        }
        ...
        if (this._internalTime >= this._lastFixedTimeStep) {
