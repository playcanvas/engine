 ...
 * component will be rendered on top of other components.
 * @property {Vec4} anchor Specifies where the left, bottom, right and top edges of the component
 * are anchored relative to its parent. Each value ranges from 0 to 1. E.g. a value of [0,0,0,0]
 * means that the element will be anchored to the bottom left of its parent. A value of
 * [1, 1, 1, 1] means it will be anchored to the top right. A split anchor is when the left-right
 * or top-bottom pairs of the anchor are not equal. In that case the component will be resized to
 * cover that entire area. E.g. a value of [0,0,1,1] will make the component resize exactly as its
 * parent.
 * @property {Vec2} pivot The position of the pivot of the component relative to its anchor. Each
 * value ranges from 0 to 1 where [0,0] is the bottom left and [1,1] is the top right.
 * @property {Vec4} margin The distance from the left, bottom, right and top edges of the anchor.
 * For example if we are using a split anchor like [0,0,1,1] and the margin is [0,0,0,0] then the
 * component will be the same width and height as its parent.
 * @property {number} left The distance from the left edge of the anchor. Can be used in
 * combination with a split anchor to make the component's left edge always be 'left' units away
 * from the left.
 * @property {number} right The distance from the right edge of the anchor. Can be used in
 * combination with a split anchor to make the component's right edge always be 'right' units away
 * from the right.
 * @property {number} bottom The distance from the bottom edge of the anchor. Can be used in
 * combination with a split anchor to make the component's top edge always be 'top' units away from
 * the top.
 * @property {number} top The distance from the top edge of the anchor. Can be used in combination
 * with a split anchor to make the component's bottom edge always be 'bottom' units away from the
 * bottom.
 * @property {number} width The width of the element as set in the editor. Note that in some cases
 * this may not reflect the true width at which the element is rendered, such as when the element
 * is under the control of a {@link LayoutGroupComponent}. See `calculatedWidth` in order to ensure
 * you are reading the true width at which the element will be rendered.
 * @property {number} height The height of the element as set in the editor. Note that in some
 * cases this may not reflect the true height at which the element is rendered, such as when the
 * element is under the control of a {@link LayoutGroupComponent}. See `calculatedHeight` in order
 * to ensure you are reading the true height at which the element will be rendered.
 * @property {number} calculatedWidth The width at which the element will be rendered. In most
 * cases this will be the same as `width`. However, in some cases the engine may calculate a
 * different width for the element, such as when the element is under the control of a
 * {@link LayoutGroupComponent}. In these scenarios, `calculatedWidth` may be smaller or larger
 * than the width that was set in the editor.
 * @property {number} calculatedHeight The height at which the element will be rendered. In most
 * cases this will be the same as `height`. However, in some cases the engine may calculate a
 * different height for the element, such as when the element is under the control of a
 * {@link LayoutGroupComponent}. In these scenarios, `calculatedHeight` may be smaller or larger
 * than the height that was set in the editor.
 * @property {Vec3[]} screenCorners An array of 4 {@link Vec3}s that represent the bottom left,
 * bottom right, top right and top left corners of the component relative to its parent
 * {@link ScreenComponent}.
 * @property {Vec3[]} worldCorners An array of 4 {@link Vec3}s that represent the bottom left,
 * bottom right, top right and top left corners of the component in world space. Only works for 3D
 * ElementComponents.
 * @property {Vec2[]} canvasCorners An array of 4 {@link Vec2}s that represent the bottom left,
 * bottom right, top right and top left corners of the component in canvas pixels. Only works for
 * screen space ElementComponents.
 * @property {boolean} useInput If true then the component will receive Mouse or Touch input
 * events.
 ...
 * @property {number} textWidth The width of the text rendered by the component. Only works for
 * {@link ELEMENTTYPE_TEXT} types.
 * @property {number} textHeight The height of the text rendered by the component. Only works for
 * {@link ELEMENTTYPE_TEXT} types.
 ...
 * @property {number} batchGroupId Assign element to a specific batch group (see
 * {@link BatchGroup}). Default value is -1 (no group).
 * @property {number[]} layers An array of layer IDs ({@link Layer#id}) to which this element
 * should belong. Don't push/pop/splice or modify this array, if you want to change it - set a new
 * one instead.
