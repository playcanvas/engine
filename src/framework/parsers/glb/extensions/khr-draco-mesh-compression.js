...
const createDracoMesh = (device, primitive, accessors, bufferViews, meshVariants, meshDefaultMaterials, promises) => {
    ...
    promises.push(
        new Promise((resolve, reject) => {
            ...
            const initialized = dracoDecode(
                bufferViews[dracoExt.bufferView].slice().buffer,
            ...
            if (!initialized) {
                const message =
                    "glTF file contains Draco compressed meshes, but the Draco decoder is not configured. Call dracoInitialize() or WasmModule.setConfig('DracoDecoderModule', ...) before loading the asset.";
