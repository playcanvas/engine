...
class AnimTransition {
     ...
     * @param {number} priority - Used to sort all matching transitions in ascending order. The
     * first transition in the list will be selected. Defaults to 0.
     * @param {object[]} conditions - A list of conditions which must pass for this transition to
     * be used. Defaults to [].
     * @param {number} exitTime - If provided, this transition will only be active for the exact
     * frame during which the source states progress passes the time specified. Given as a
     * normalized value of the source states duration. Values less than 1 will be checked every
     * animation loop. Defaults to null.
     * @param {number} transitionOffset - If provided, the destination state will begin playing its
     * animation at this time. Given in normalized time, based on the states duration & must be
