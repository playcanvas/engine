 ...
 *
 * A frustum is six {@link Plane}s, read and written with {@link Frustum#getPlane} and
 * {@link Frustum#setPlane}, and normally derived from a camera's combined view-projection matrix
 * with {@link Frustum#setFromMat4}. {@link Frustum#containsPoint} and {@link Frustum#containsAabb}
 * return a boolean. {@link Frustum#containsSphere} returns 0 for a sphere outside, 1 for one that
 * intersects and 2 for one fully inside, so callers can skip finer tests for objects that are
 * entirely visible. None of the tests allocate.
 *
 * @example
