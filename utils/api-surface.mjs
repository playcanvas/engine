...
function typeStr(t) {
    ...
    switch (t.type) {
        case 'intrinsic':
            return t.name;
        case 'literal':
            return typeof t.value === 'string' ? JSON.stringify(t.value) : `${t.value}`;
        case 'reference':
            return t.name + (t.typeArguments?.length ? `<${t.typeArguments.map(typeStr).join(', ')}>` : '');
        case 'array':
            return `${typeStr(t.elementType)}[]`;
        case 'union':
            return t.types.map(typeStr).join(' | ');
        case 'intersection':
            return t.types.map(typeStr).join(' & ');
        case 'tuple':
            return `[${(t.elements || []).map(typeStr).join(', ')}]`;
        case 'query':
            return `typeof ${typeStr(t.queryType)}`;
        case 'indexedAccess':
            return `${typeStr(t.objectType)}[${typeStr(t.indexType)}]`;
        case 'typeOperator':
            return `${t.operator} ${typeStr(t.target)}`;
        case 'reflection':
            return reflStr(t.declaration);
...
function member(r) {
    ...
    if (r.kind === METHOD)
        return r.signatures.map((s) => `${pre}${r.name}${typeParams(s)}(${params(s)}): ${typeStr(s.type)}`);
    if (r.kind === ACCESSOR) {
        const out = [];
...
function decl(r) {
    switch (r.kind) {
        ...
        case INTERFACE: {
            ...
            return [
                `${kw} ${r.name}${typeParams(r)}${ext}`,
                ...(r.children || []).flatMap(member).map((m) => `${r.name}.${m}`)
            ];
        }
        case FUNCTION:
            return (r.signatures || []).map(
                (s) => `function ${r.name}${typeParams(s)}(${params(s)}): ${typeStr(s.type)}`
            );
        case VARIABLE:
            return [`const ${r.name}: ${typeStr(r.type)}`];
        case TYPE_ALIAS:
            return [`type ${r.name}${typeParams(r)} = ${typeStr(r.type)}`];
        case ENUM:
            return [`enum ${r.name}`, ...(r.children || []).map((c) => `${r.name}.${c.name}`)];
        case NAMESPACE:
            return [
                `namespace ${r.name}`,
                ...(r.children || [])
                    .filter((c) => TOP.includes(c.kind))
                    .flatMap(decl)
                    .map((l) => `${r.name}.${l}`)
            ];
        default:
