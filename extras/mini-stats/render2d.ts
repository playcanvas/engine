...
class Render2d {
    ...
    prims: { type: number, base: number, count: number, indexed?: boolean, texture?: pc.Texture }[];
    prim: { type: number, base: number, count: number, indexed?: boolean, texture?: pc.Texture };
    primIndex: number;
    quads: number;
    background: Float32Array;
    backgroundId: pc.ScopeId;
    clr: Float32Array;
    clrId: pc.ScopeId;
    col0: Float32Array;
    col0Id: pc.ScopeId;
    ...
    screenTextureSizeId: pc.ScopeId;
    watermark: Float32Array;
    watermarkId: pc.ScopeId;
    watermarkSizeId: pc.ScopeId;

    constructor(device: pc.GraphicsDevice, colors: {[key: string]: pc.Color}, maxQuads?: number) {
        ...
        this.data = new Float32Array(maxQuads * 4 * 7);
    ...
    quad(texture: pc.Texture, x: number, y: number, w: number, h: number, u: number, v: number, uw: number, uh: number, enabled: boolean): void {
        var quad = this.quads++;
    ...
    render(clr: number[], height: number) {
