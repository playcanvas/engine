 ...
 * @param {any} script - The script using the effect.
 */
function SSAOEffect(graphicsDevice, script) {
    ...
    this.ssaoShader = new pc.Shader(graphicsDevice, {
        ...
        vshader: `${graphicsDevice.webgl2 ? "#version 300 es" : ""}
            ${graphicsDevice.webgl2 ? pc.shaderChunks.gles3VS : ""}
        ...
        fshader: `${graphicsDevice.webgl2 ? "#version 300 es" : ""}
            ${graphicsDevice.webgl2 ? pc.shaderChunks.gles3PS : ""}
            ${!graphicsDevice.webgl2 ? "#extension GL_OES_standard_derivatives: enable" : ""}
            ...
            void computeAmbientOcclusionSAO(inout float occlusion, float i, float ssDiskRadius, const highp vec2 uv,  const highp vec3 origin, const vec3 normal, const vec2 tapPosition, const float noise) {
            ...
            float scalableAmbientObscurance(highp vec2 uv, highp vec3 origin, vec3 normal) {
                ...
                ${graphicsDevice.webgl2 ? "for (float i = 0.0; i < uSampleCount.x; i += 1.0) {" : ""}

                // webgl1 does not handle non-constant loop, work around it
                ${!graphicsDevice.webgl2 ? "const float maxSampleCount = 256.0;" : ""}
                ${!graphicsDevice.webgl2 ? "for (float i = 0.0; i < maxSampleCount; i += 1.0) {" : ""}
                    ${!graphicsDevice.webgl2 ? "if (i >= uSampleCount.x) break;" : ""}
    ...
    this.blurShader = new pc.Shader(graphicsDevice, {
        ...
        fshader: `${graphicsDevice.webgl2 ? "#version 300 es" : ""}
            ...
            void main() {
                ...
                // webgl1 does not handle non-constant loop, work around it
                ${graphicsDevice.webgl2 ? "for (int x = -uBilatSampleCount; x <= uBilatSampleCount; x++) {" : ""}
                    ${graphicsDevice.webgl2 ? "for (int y = -uBilatSampleCount; y < uBilatSampleCount; y++) {" : ""}

                ${!graphicsDevice.webgl2 ? "for (int x = -4; x <= 4; x++) {" : ""}
                    ${!graphicsDevice.webgl2 ? "for (int y = -4; y < 4; y++) {" : ""}
...
SSAOEffect.prototype.render = function (inputTarget, outputTarget, rect) {
    var device = this.device;
    var scope = device.scope;

    var sampleCount = this.script.samples;
    var spiralTurns = 10.0;
    var step = (1.0 / (sampleCount - 0.5)) * spiralTurns * 2.0 * 3.141;

    var radius = this.script.radius;
    var bias = 0.001;
    var peak = 0.1 * radius;
    ...
    scope.resolve("uBrightness").setValue(this.script.brightness);
...
SSAO.attributes.add('downSample', {
    type: 'number',
    default: 1,
    min: 1,
    title: 'Down Sample'
