...
// brightness-varied hash stars, and a soft moon disk. Blended in by procSkyNightBlend. Included
// in both the visible sky and the IBL bake so night ambient reaches scene objects.
...
const EQUIRECT_FS_GLSL = /* glsl */ `
    ...
    void main(void) {
        ...
        gl_FragColor = encodeSkyRGBM(linear);
    }
`;
...
const EQUIRECT_FS_WGSL = /* wgsl */ `
    ...
    fn fragmentMain(input: FragmentInput) -> FragmentOutput {
        ...
        output.color = encodeSkyRGBM(linear);
        return output;
    }
`;
...
class ProceduralSky extends Script {
    ...
    initialize() {
        ...
        // NOTE: this is a device-wide override — only one ProceduralSky instance may be active
        // per graphics device at a time. A second instance would capture the first's override as
        // its "original" and restore the wrong chunk on destroy.
