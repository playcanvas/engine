...
var scaleCompensatePos = new Vec3(0, 0, 0);
var scaleCompensateRot = new Quat(0, 0, 0, 1);
var scaleCompensateRot2 = new Quat(0, 0, 0, 1);
var scaleCompensateScale = new Vec3(0, 0, 0);
var scaleCompensateScaleForParent = new Vec3(0, 0, 0);
var tmpMat4 = new Mat4();
var tmpQuat = new Quat(0, 0, 0, 1);
...
export class GraphNode {
     ...
     */
    _onHierarchyStateChanged(enabled: boolean): void {
    ...
    _cloneInternal(clone: GraphNode): void {
    ...
    */
    ...
    getWorldTransform(): Mat4 {
        ...
        if (this._parent) {
            // exclamation mark because of false negative null guard checker
     ...
     * @param {i32} [index] - The child index where the child node should be placed.
     */
    reparent(parent: GraphNode, index: i32): void {
    ...
    setPosition_position: Vec3 = new Vec3(0, 0, 0);
    setPosition_invParentWtm: Mat4 = new Mat4();

    //setPosition() {
    //    var position = new Vec3();
    //    var invParentWtm = new Mat4();

        setPosition(x: Vec3): void {
        //setPosition(x: number, y: number, z: number): void {
            // if (x instanceof Vec3) {
                this.setPosition_position.copy(x);
            // } else {
            //     this.setPosition_position.set(x, y, z);
            // }

            if (this._parent === null) {
                this.localPosition.copy(this.setPosition_position);
            } else {
                this.setPosition_invParentWtm.copy(this._parent!.getWorldTransform()).invert();
                this.setPosition_invParentWtm.transformPoint(this.setPosition_position, this.localPosition);
            }
     ...
     */
    setRotation_rotation: Quat = new Quat(0, 0, 0, 1);
    setRotation_invParentRot: Quat = new Quat(0, 0, 0, 1);
    // setRotation() {
        ...
        setRotation(x: Quat): void {
            // if (x instanceof Quat) {
                this.setRotation_rotation.copy(x);
            // } else {
            //    this.setRotation_rotation.set(x, y, z, w);
            // }

            if (this._parent === null) {
                this.localRotation.copy(this.setRotation_rotation);
            } else {
                var parentRot = this._parent!.getRotation();
                this.setRotation_invParentRot.copy(parentRot).invert();
                this.localRotation.copy(this.setRotation_invParentRot).mul(this.setRotation_rotation);
            }
     ...
     * this.entity.setEulerAngles(angles);
     */
    setEulerAngles_invParentRot: Quat = new Quat(0, 0, 0, 1);
    //setEulerAngles() {
        //var invParentRot = new Quat();
        ...
        setEulerAngles(x: number, y: number, z: number): void {
            ...
            if (this._parent !== null) {
                var parentRot = this._parent!.getRotation();
                this.setEulerAngles_invParentRot.copy(parentRot).invert();
                this.localRotation.mul2(this.setEulerAngles_invParentRot, this.localRotation);
            }
    ...
    /*
    addChildAndSaveTransform(node: GraphNode) {
        // #ifdef DEBUG
        this._debugInsertChild(node);
        // #endif
    ...
    */
     ...
     * @param {i32} index - The index in the child list of the parent where the new node will be inserted.
     * @example
     * var e = new pc.Entity(app);
     * this.entity.insertChild(e, 1);
     */
    insertChild(node: GraphNode, index: i32): void {
        if (node._parent !== null)
            throw new Error("GraphNode is already parented");
        ...
        this._children = insertAfter(this._children, index, node);
        this._onInsertChild(node);
    }

    // #ifdef DEBUG
    _debugInsertChild(node: GraphNode): void {
        if (this === node)
    ...
    _onInsertChild(node: GraphNode): void {
        node._parent = this;
                ...
                if (this.scaleCompensation) {
                    ...
                    if (parentToUseScaleFrom) {
                        ...
                        if (parentToUseScaleFrom) {
                            ...
                            if (parentToUseScaleFrom) {
                                parentWorldScale = parentToUseScaleFrom.worldTransform.getScale(this._sync_scale);
     ...
     */
    lookAt_matrix: Mat4 = new Mat4();
    lookAt_target: Vec3 = new Vec3(0, 0, 0);
    lookAt_up: Vec3 = new Vec3(0, 0, 0);
    lookAt_rotation: Quat = new Quat(0, 0, 0, 1);
    //lookAt() {
    //    var matrix = new Mat4();
    //    var target = new Vec3();
    //    var up = new Vec3();
    //    var rotation = new Quat();
        ...
        lookAt(tx: number, ty: number, tz: number, ux: number, uy: number, uz: number): void {
            ...
            //     return;
            // } else {
                this.lookAt_target.set(tx, ty, tz);

                // if (ux !== undefined) { // number, number, number, number, number, number
                    this.lookAt_up.set(ux, uy, uz);
                // } else { // number, number, number
                //     up.copy(Vec3.UP);
                // }
            // }

            this.lookAt_matrix.setLookAt(this.getPosition(), this.lookAt_target, this.lookAt_up);
            this.lookAt_rotation.setFromMat4(this.lookAt_matrix);
            this.setRotation(this.lookAt_rotation);
        }
    //}()
     ...
     */
    translate_translation: Vec3 = new Vec3(0, 0, 0);
    ...
    //translate() {
        ...
        translate(x: number, y: number, z: number): void {
            ...
            //     translation.copy(x);
            // } else {
                this.translate_translation.set(x, y, z);
            // }

            this.translate_translation.add(this.getPosition());
            this.setPosition(this.translate_translation);
        }
    //}()
     ...
     */
    translateLocal_translation: Vec3 = new Vec3(0, 0, 0);
    //translateLocal() {
        //var translation = new Vec3();

        /*return function*/ translateLocal(x: number, y: number, z: number): void {
            // if (x instanceof Vec3) {
            //     translation.copy(x);
            // } else {
                this.translateLocal_translation.set(x, y, z);
            // }

            this.localRotation.transformVector(this.translateLocal_translation, this.translateLocal_translation);
            this.localPosition.add(this.translateLocal_translation);
     ...
     */
    rotate_quaternion: Quat = new Quat(0, 0, 0, 1);
    rotate_invParentRot: Quat = new Quat(0, 0, 0, 1);
    // rotate() {
    ...
    //     var invParentRot = new Quat();
        ...
        rotate(x: number, y: number, z: number): void {
            ...
            // } else {
                this.rotate_quaternion.setFromEulerAngles(x, y, z);
            //}

            if (this._parent === null) {
                this.localRotation.mul2(this.rotate_quaternion, this.localRotation);
            } else {
                var rot = this.getRotation();
                var parentRot = this._parent!.getRotation();

                this.rotate_invParentRot.copy(parentRot).invert();
                this.rotate_quaternion.mul2(this.rotate_invParentRot, this.rotate_quaternion);
                this.localRotation.mul2(this.rotate_quaternion, rot);
            }
     ...
     */
    rotateLocal_quaternion: Quat = new Quat(0, 0, 0, 1);
    // rotateLocal() {
        // var quaternion = new Quat();
        ...
        rotateLocal(x: number, y: number, z: number): void {
            ...
            //     quaternion.setFromEulerAngles(x.x, x.y, x.z);
            // } else {
                this.rotateLocal_quaternion.setFromEulerAngles(x, y, z);
            // }

            this.localRotation.mul(this.rotateLocal_quaternion);
