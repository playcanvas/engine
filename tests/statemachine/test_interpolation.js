...
test('Smooth returns correctly the interpolation of two numbers', function () {

	equal(pc.interpolate.smooth(0, 10, 0.5), 5, "Middle interpolated");
	equal(pc.interpolate.smooth(0, 10, 0), 0, "start interpolated");
	equal(pc.interpolate.smooth(0, 10, 1), 10, "end interpolated");
	equal(pc.interpolate.smooth(0, 10, 1.2), 10, "can interpolate out of range");
...
asyncTest('Smooth over time', function () {

	var v = -1;
	var v2 = -1;
	pc.interpolate.overTime.smooth(0, 10, 2).on('value', function (value) {
		v = value;
	});
	pc.interpolate.overTime.smooth(0, 10, 2, function (value) {
		v2 = value;
	});
	setTimeout(function () {
		ok(Math.abs(v - 5) < 0.25, "Event driven: " + v);
